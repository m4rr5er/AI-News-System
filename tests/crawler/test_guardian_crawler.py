"""
Test script for The Guardian News crawler
Tests the ability to crawl The Guardian News and extract content, saving results to JSON file
"""
...
from scrapers.spiders.guardian_spider import GuardianSpider


class GuardianTestPipeline:
    ...
    def __init__(self):
        ...
        self.output_file = os.path.join(
            ...
            f'guardian_test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )

    def open_spider(self, spider):
        """Called when spider is opened"""
        print(f"\n{'='*60}")
        print(f"The Guardian News Crawler Test Started")
        print(f"{'='*60}\n")
    ...
    def close_spider(self, spider):
        ...
        print(f"\n{'='*60}")
        print(f"The Guardian News Crawler Test Completed")
        print(f"{'='*60}")
...
def run_test():
    ...
    settings.update({
        'ITEM_PIPELINES': {
            '__main__.GuardianTestPipeline': 300,
        },
        'LOG_LEVEL': 'INFO',
    })
    ...
    print("\nStarting The Guardian crawler test...")
