"""
Test script for China Daily News crawler
Tests the ability to crawl China Daily News and extract content, saving results to JSON file
"""
...
from scrapers.spiders.chinadaily_spider import ChinaDailySpider


class ChinaDailyTestPipeline:
    ...
    def __init__(self):
        ...
        self.output_file = os.path.join(
            ...
            f'chinadaily_test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )

    def open_spider(self, spider):
        """Called when spider is opened"""
        print(f"\n{'='*60}")
        print(f"China Daily News Crawler Test Started")
        print(f"{'='*60}\n")
    ...
    def close_spider(self, spider):
        ...
        print(f"\n{'='*60}")
        print(f"China Daily News Crawler Test Completed")
        print(f"{'='*60}")
...
def run_test():
    ...
    settings.update({
        'ITEM_PIPELINES': {
            '__main__.ChinaDailyTestPipeline': 300,
        },
        'LOG_LEVEL': 'INFO',
    })
    ...
    print("\nStarting China Daily crawler test...")
